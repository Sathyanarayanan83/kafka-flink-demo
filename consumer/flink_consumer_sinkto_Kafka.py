...
PyFlink Consumer — reads JSON events from:
  - user-events    → counts events per type, running total
  - user-activity  → filters purchases, computes total spend per user

Both result streams are written as JSON to:
  - aggregated-results
...
TOPIC_RESULTS = "aggregated-results"
...
class EventCountToJson(MapFunction):
    """Serialize (event_type, count) → JSON string for the results topic."""
...
class UserSpendToJson(MapFunction):
    """Serialize (user_id, total_spend) → JSON string for the results topic."""

    def map(self, value):
        user_id, total_spend = value
        return json.dumps({
            "metric": "user_total_spend",
            "user_id": user_id,
...
def main():
    ...
    results_sink = build_kafka_sink(TOPIC_RESULTS)

    # Write event-type counts to the results topic
    event_counts.map(EventCountToJson(), output_type=Types.STRING()).sink_to(results_sink)
    ...
    # Write user spend totals to the results topic
    user_spend.map(UserSpendToJson(), output_type=Types.STRING()).sink_to(results_sink)
    ...
    print(f"  Writing results to '{TOPIC_RESULTS}'")
    print("=" * 60)
